...
description: How to query SCD Type 2 history tables effectively, including current state queries, point-in-time analysis, and change tracking.
sidebar:
  order: 3
---
...
`__START_AT``__END_AT``START_AT``END_AT`
...
`TRACK HISTORY ON specific_columns`

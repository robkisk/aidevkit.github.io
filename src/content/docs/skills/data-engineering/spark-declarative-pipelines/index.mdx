...
AUTO CDC tracks the full change history. Query current state with `WHERE __END_AT IS NULL`. For point-in-time analysis, filter with `WHERE __START_AT &lt;= '2025-06-01' AND (__END_AT IS NULL OR __END_AT &gt; '2025-06-01')`. Note the double underscores on `__START_AT` and `__END_AT`. Put `APPLY AS DELETE WHEN` before `SEQUENCE BY` — the parser is strict about clause order.

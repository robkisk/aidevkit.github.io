...
Checkpoints store the offset and state metadata that give Structured Streaming its exactly-once guarantees. Getting checkpoint storage, naming, and lifecycle management right prevents data loss, duplicate processing, and hard-to-debug recovery failures.

...
Stream-stream joins correlate events from two real-time sources -- for example matching orders to payments or clicks to impressions. They require watermarks and bounded time conditions to control state size, making them more complex than stream-static joins but essential for event-time correlation at scale.

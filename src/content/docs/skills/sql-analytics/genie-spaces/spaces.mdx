...
**Key decisions:**
- **Always inspect tables first** -- `get_table_stats_and_schema` returns column names, types, sample values, and cardinality. Sample questions that reference actual column names produce dramatically better SQL generation.
- **Single `manage_genie` MCP tool** -- one tool covers create, update, get, list, delete, export, and import via the `action` parameter. Pair it with `ask_genie` for queries.
...
`manage_genie(action="create_or_update")` handles both create and update. Pass `space_id` to target a specific space, or omit it to match by `display_name`. If no match is found, a new space is created.
...
The export envelope contains `space_id`, `title`, `description`, `warehouse_id`, and `serialized_space` (a JSON-encoded string with the full config -- tables, sample questions, certified SQL queries, join specs, filter snippets, and benchmarks). The import creates a brand new space with a new `space_id`.
...
for sid in space_ids:
    exported = manage_genie(action="export", space_id=sid)
...
- **`manage_genie(action="export")` returns empty `serialized_space`** -- this means the caller lacks CAN EDIT permission on the source space. Check space permissions.

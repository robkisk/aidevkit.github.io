...
**Key decisions:**
- **`PAGE_TYPE_GLOBAL_FILTERS` + `layoutVersion: GRID_V1`** -- a dedicated filter page declares both. The 12-column grid with `GRID_V1` is mandatory; without it, position widths and heights are ignored.
...
- **Width 4, height 2** -- standard filter sizing on the 12-column grid. Three filters fit per row (4+4+4=12).
...
- **Missing `layoutVersion: GRID_V1`** -- both global filter pages and canvas pages with page-level filters need `"layoutVersion": "GRID_V1"` next to `pageType`. Without it, the filter renders on the legacy grid and ignores the position widths.

...
- **Using `ARRAY()` syntax instead of JSON strings** -- `ai_classify` and `ai_extract` now accept JSON string arguments, not SQL `ARRAY()`. Use `'["a", "b"]'` for simple labels and `'\{"key": "description"\}'` for labeled maps.

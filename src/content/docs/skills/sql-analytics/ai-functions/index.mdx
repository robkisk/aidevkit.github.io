...
- **`ai_extract` returns a STRUCT** -- access nested fields with dot notation: `entities.product`, `entities.error_code`. No JSON parsing needed.
- **`ARRAY()` for labels and fields** -- both `ai_classify` and `ai_extract` accept SQL arrays. Keep labels clear and mutually exclusive for reliable results.

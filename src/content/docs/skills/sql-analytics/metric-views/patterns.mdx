...
Window measures enable trailing aggregations, running totals, and period-over-period calculations. They are still an experimental feature and require `version: 0.1` even when the rest of your metric views run on `1.1`. `semiadditive: last` controls what happens when the window dimension isn't in the GROUP BY -- it returns the last value instead of summing across time. Derived measures reference window measures via `MEASURE()` and do not need their own `window` block.

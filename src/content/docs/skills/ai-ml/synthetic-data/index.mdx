...
You can generate realistic synthetic datasets that tell a business story — something goes wrong, it costs money, the data lets you trace root cause and act on it. Your AI coding assistant designs the schema with realistic distributions, referential integrity, and time patterns, then generates a script that creates infrastructure, produces data, and writes to Unity Catalog in one execution. The output is ready for dashboards, SDP pipelines, Genie Spaces, or Knowledge Assistant demos — and it actually demonstrates Databricks value because the data has anomalies, cohorts, and dollar impact you can find.

## In Action

> "Generate a support demo with 10K customers and 80K tickets. Include a payment outage that spikes tickets, degrades resolution times, and causes enterprise customers to churn. Write to my Unity Catalog."
...
**Key decisions:**
- **Story-driven generation** — every table connects to a business event. The outage spikes tickets, the ticket spike degrades resolution times, slow resolution drives enterprise churn. Without that causal chain, the data has no analysis hook and can't show Databricks value.
...
## Watch Out For
- **Flat data has no story** — uniform random distributions produce datasets with no spikes, cohorts, or anomalies, which means there is nothing to analyze and nothing to demo. Always skew categories (60/30/10 not 33/33/33) and use log-normal or Pareto for monetary values.
- **No Python loops or `.collect()`** — driver-side iteration defeats Spark parallelism. Stay in DataFrame transforms, use `spark.range()` for row generation, and reach for `pandas_udf` only when you need Python libraries like Faker.
- **Never use `.cache()` or `.persist()` on serverless** — throws `AnalysisException: PERSIST TABLE is not supported on serverless compute`. Write master tables to Delta first, then read them back for joins. Broadcast variables and `spark.sparkContext` are also unsupported.

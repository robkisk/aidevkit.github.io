---
title: Data Patterns Guide
description: Creating realistic, coherent synthetic data with Spark + Pandas UDFs.
sidebar:
  order: 3
---
...
`np.random.lognormal(mean=4.5, sigma=0.8)`
`(np.random.pareto(a=2.5) + 1) * 10`
`np.random.exponential(scale=24)`
`np.random.choice(vals, p=[0.4, 0.3, 0.2, 0.1])`

...
```python
# Storage-Optimized endpoint with SQL-like filters (databricks-vectorsearch package)
from databricks.vector_search.client import VectorSearchClient

vsc = VectorSearchClient()
index = vsc.get_index(endpoint_name="rag-endpoint", index_name="catalog.schema.kb_index")

results = index.similarity_search(
    query_text="data access controls",
    columns=["doc_id", "title", "content"],
    num_results=5,
    filters="category = 'governance' AND updated_at >= '2024-01-01'"
)

# Standard endpoint with dict-format filters (databricks-sdk):
# results = w.vector_search_indexes.query_index(
#     index_name="catalog.schema.kb_index",
#     columns=["doc_id", "title", "content"],
#     query_text="data access controls",
...
The filter parameter name and syntax depend on which package and endpoint type you use. Storage-Optimized endpoints take SQL-like strings via `filters` in `databricks-vectorsearch`. Standard endpoints take JSON dicts via `filters_json` in `databricks-sdk`. Mismatching them silently returns wrong results.
...
- **Mixing up `filters_json` and `filters`** -- Standard endpoints use the dict-based `filters_json` (in `databricks-sdk`). Storage-Optimized use SQL-like `filters` strings (in `databricks-vectorsearch`). The error message when you mix them is not obvious.

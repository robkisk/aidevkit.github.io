...
- **Wrong filter syntax for your endpoint type** -- Standard endpoints use `filters_json` (dict format) in `databricks-sdk`. Storage-Optimized endpoints use `filters` (SQL-like string) in `databricks-vectorsearch`. Using the wrong one returns an error that doesn't clearly explain the issue.

...
description: Ship Python web apps on Databricks with OAuth, SQL warehouse queries, Lakebase persistence, and model serving — using Streamlit, Dash, FastAPI, Gradio, Flask, or Reflex.
sidebar:
...
:::tip[Considering AppKit instead?]
For brand-new apps, the Databricks-recommended SDK is now [AppKit](/aidevkit.github.io/skills/apps-databases/databricks-apps-appkit/) — a TypeScript + React SDK with plugins for SQL, Genie, Files, and Lakebase. Reach for Python when your team is Python-only, you need Streamlit/Dash/Gradio specifically, or you are extending an existing Python app.
:::

## What You Can Build

You can deploy production Python web apps directly on the Databricks platform — dashboards with Dash, prototypes with Streamlit, ML demos with Gradio, REST APIs with FastAPI or Flask, or full-stack reactive UIs with Reflex. Each app gets OAuth authentication, resource bindings for SQL warehouses and Lakebase, and access to model serving endpoints. Ask your AI coding assistant for a specific framework and use case and it will generate the app code, `app.yaml` config, and deployment commands in one pass. For full recipes, see the [Databricks Apps Cookbook](https://apps-cookbook.dev/).

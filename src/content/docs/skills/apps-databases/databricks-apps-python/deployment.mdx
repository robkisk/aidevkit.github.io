...
```python
# The manage_workspace_files MCP tool (action="upload") auto-skips:
...
- **Uploading `node_modules` or virtualenvs via `workspace import-dir`** -- the CLI uploader does not filter build artifacts. Clean the directory first or use the `manage_workspace_files` MCP tool (`action="upload"`), which auto-skips `node_modules`, `__pycache__`, `.venv`, `dist`, `build`, and similar directories.

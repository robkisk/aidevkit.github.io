...
- **Assuming the `public` schema works** -- the Lakebase service principal often cannot access the default `public` schema. Create a custom schema (e.g., `app_data`) at startup and use it for all tables. This is a common source of permission-denied errors that look like driver issues.
